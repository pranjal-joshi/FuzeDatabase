<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		$dataArray[0] = concatPcbBatch($dataArray[0],$_COOKIE['fuzeType'],$_COOKIE['fuzeDia'],"PCB",$db);
		...
		$recordDateSql = "UPDATE `pcb_testing` SET `record_date` = STR_TO_DATE('".$_POST['record_date']."', '%e %M, %Y') WHERE `pcb_no` = '".$dataArray[0]."'";
		...
		if(!$res || !$dateRes) {
			...
			// assign new dummy lot to added PCB - MainLot=0, KitLot=PCB
