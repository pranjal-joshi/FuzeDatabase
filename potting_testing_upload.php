<?php
	...
	require('library/SpreadsheetReader.php');
		...
		if(strtoupper($array[23]) == "FAIL") {
			...
			/*
			Not applicable for epoxy potting stage
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if($_FILES['file']['type'] == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
			...
			for($i=0;$i<$totalSheet;$i++){
				...
				$sqlAutoIncReset = "ALTER TABLE `potting_table` DROP `_id`;";
				$autoIncResult = mysqli_query($db, $sqlAutoIncReset);

				$sqlAdd = "REPLACE INTO `potting_table` (`pcb_no`, `i`, `vee`, `vbat_pst`, `pst_amp`, `pst_wid`, `mod_freq`, `mod_dc`, `mod_ac`, `cap_charge`, `vrf_amp`, `vbat_vrf`, `vbat_sil`, `det_wid`, `det_amp`, `cycles`, `bpf_dc`, `bpf_ac`, `sil`, `lvp`, `pd_delay`, `pd_det`, `safe`, `result`, `op_name`, `record_date`,`bpf_noise_ac`,`bpf_noise_dc`) VALUES ";
				...
				$sqlAutoIncReset = "ALTER TABLE `potting_table` ADD `_id` INT NOT NULL AUTO_INCREMENT FIRST, ADD PRIMARY KEY (`_id`);";
				$autoIncResult = mysqli_query($db, $sqlAutoIncReset);
