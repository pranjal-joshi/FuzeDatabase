	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		$html = "
			...
			$html.="
				...
				$addSql = "REPLACE INTO `potted_time_csv` (`_id`, `pcb_no`, `op_id`, `tester_id`, `assy_stage`, `record_date`, `record_time`, `partial_test`, `t1_status`, `vbat_v`, `vbat_i`, `tpcd_delay`, `pst_delay`, `pst_amp`, `pst_width`, `3_delay`, `3_amp`, `3_width`, `t2_status`, `16_delay`, `16_amp`, `16_width`, `t3_status`, `24_delay`, `24_amp`, `24_width`, `t4_status`, `pd_det`, `t5_status`, `safe_pst`, `safe_det`, `result`) VALUES ";
				...
				for($cnt=2;$cnt<=count($csvArray)-2;$cnt++) {					// auto add to rejection while uploading CSV
					...
					$lot_no = $dataArray[2];
					$pcb_no = substr($z,8,6);
					addTimeCsvRejection($pcb_no,$dataArray,"POTTED",$db);
				}

				$sqlAutoIncReset = "ALTER TABLE `potted_time_csv` DROP `_id`;";
				$autoIncResult = mysqli_query($db, $sqlAutoIncReset);

				$sqlAutoIncReset = "ALTER TABLE `potted_time_csv` ADD `_id` INT NULL DEFAULT NULL AUTO_INCREMENT FIRST, ADD PRIMARY KEY (`_id`);";
				$autoIncResult = mysqli_query($db, $sqlAutoIncReset);
