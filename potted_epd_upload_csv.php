	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		$html = "
			...
			$html.="
				...
				$addSql = "REPLACE INTO `potted_epd_csv` (`_id`, `pcb_no`, `op_id`, `tester_id`, `assy_stage`, `record_date`, `record_time`, `partial_test`, `t1_status`, `vbat_v`, `vbat_i`, `vdd`, `tpcd_delay`, `pst_delay`, `pst_amp`, `pst_width`, `pd_delay`, `pd_amp`, `pd_width`, `t2_status`, `delay_delay`, `delay_amp`, `delay_width`, `t3_status`, `si_mode`, `si_delay`, `si_amp`, `si_width`, `t4_status`, `safe_pst`, `safe_det`, `result`) VALUES ";
				...
				for($cnt=1;$cnt<=count($csvArray)-2;$cnt++) {					// auto add to rejection while uploading CSV
					...
					$pcb_no = substr($z,12);
					addEpdCsvRejection($pcb_no,$dataArray,"POTTED HOUSING",$db);
				}
